    ...
    private void initHandlerMapping() {
        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                String[] methodSecurity = null; //方法上的权限
                if(method.isAnnotationPresent(Security.class)){
                    Security security = method.getAnnotation(Security.class);
                    methodSecurity = security.value();
                }
                ...
                Handler handler = new Handler(entry.getValue(),method, Pattern.compile(url),AddStringArr(baseSecurity,methodSecurity));
...
//        method.invoke() //
        ...
        if(handler.getParamIndexMapping().get(HttpServletRequest.class.getSimpleName())!=null){
            int requestIndex = handler.getParamIndexMapping().get(HttpServletRequest.class.getSimpleName()); // 0
            paraValues[requestIndex] = req;
        }

        if(handler.getParamIndexMapping().get(HttpServletResponse.class.getSimpleName())!=null){
        ...
        try {
            resp.getWriter().write(handler.getMethod().invoke(handler.getController(),paraValues).toString());
        } catch (IllegalAccessException e) {
            e.printStackTrace();
    ...
    private Handler getHandler(HttpServletRequest req) {
        ...
        if(username!=null&&!"".equals(username)){
            ...
            for(Handler handler: handlerMapping) {
                ...
                if(!matcher.matches()||!handler.getSecurity().contains(username)){continue;}
                return handler;
            }
        }
    ...
    //合并两个数组,返回set集合
    private Set<String > AddStringArr(String[] arr1,String[] arr2){
        String arrString = "";
        if(arr1!=null){
            for(String str:arr1){
                arrString += str+",";
        ...
        Set<String > set = new HashSet<>(Arrays.asList(arrString.split(",")));
        return set;
    }
